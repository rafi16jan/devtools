import{ObjectWrapper,Settings,UIString}from'../common/common.js';import{NumberUtilities,ArrayUtilities}from'../platform/platform.js';import{Utils,ARIAUtils,Icon,UIUtils,InplaceEditor,ContextMenu,Widget}from'../ui/ui.js';class DataGridImpl extends ObjectWrapper.ObjectWrapper{constructor(dataGridParameters){super();const{displayName,columns:columnsArray,editCallback,deleteCallback,refreshCallback}=dataGridParameters;this.element=createElementWithClass('div','data-grid');Utils.appendStyle(this.element,'data_grid/dataGrid.css');this.element.tabIndex=0;this.element.addEventListener('keydown',this._keyDown.bind(this),false);this.element.addEventListener('contextmenu',this._contextMenu.bind(this),true);this.element.addEventListener('focusin',event=>{this.updateGridAccessibleNameOnFocus();event.consume(true);});this.element.addEventListener('focusout',event=>{this.updateGridAccessibleName('');event.consume(true);});ARIAUtils.markAsApplication(this.element);this._displayName=displayName;this._editCallback=editCallback;this._deleteCallback=deleteCallback;this._refreshCallback=refreshCallback;const headerContainer=this.element.createChild('div','header-container');this._headerTable=headerContainer.createChild('table','header');ARIAUtils.markAsHidden(this._headerTable);this._headerTableHeaders={};this._scrollContainer=this.element.createChild('div','data-container');this._dataTable=this._scrollContainer.createChild('table','data');this._ariaLiveLabel=this.element.createChild('div','aria-live-label');ARIAUtils.markAsPoliteLiveRegion(this._ariaLiveLabel,false);if(editCallback){this._dataTable.addEventListener('dblclick',this._ondblclick.bind(this),false);}
...
setFocusable(focusable){this.element.tabIndex=focusable?0:-1;if(focusable===false){ARIAUtils.removeRole(this.element);}}
...
if(column.sortable){cell.addEventListener('click',this._clickInHeaderCell.bind(this),false);cell.classList.add('sortable');const icon=Icon.Icon.create('','sort-order-icon');cell.createChild('div','sort-order-icon-container').appendChild(icon);cell[DataGrid._sortIconSymbol]=icon;}}
...
const column=this.visibleColumnsArray[cellIndex];if(column.dataType===DataGrid.DataGrid.DataType.Boolean){const checkboxLabel=UIUtils.CheckboxLabel.create(undefined,(node.data[column.id]));ARIAUtils.setAccessibleName(checkboxLabel,column.title||'');let hasChanged=false;checkboxLabel.style.height='100%';const checkboxElement=checkboxLabel.checkboxElement;checkboxElement.classList.add('inside-datagrid');const initialValue=checkboxElement.checked;checkboxElement.addEventListener('change',()=>{hasChanged=true;this._editingCommitted(element,checkboxElement.checked,initialValue,undefined,'forward');},false);checkboxElement.addEventListener('keydown',event=>{if(event.key==='Tab'){event.consume(true);hasChanged=true;return this._editingCommitted(element,checkboxElement.checked,initialValue,undefined,event.shiftKey?'backward':'forward');}
...
this._editingCommitted(element,checkboxElement.checked,checkboxElement.checked,undefined,'next');},false);element.innerHTML='';element.appendChild(checkboxLabel);checkboxElement.focus();}else{InplaceEditor.InplaceEditor.startEditing(element,this._startEditingConfig(element));element.getComponentSelection().selectAllChildren(element);}}
...
_startEditingConfig(element){return new InplaceEditor.Config(this._editingCommitted.bind(this),this._editingCancelled.bind(this));}
...
setName(name){this._columnWeightsSetting=Settings.Settings.instance().createSetting('dataGrid-'+name+'-columnWeights',{});this._loadColumnWeights();}
...
_keyDown(event){if(event.shiftKey||event.metaKey||event.ctrlKey||this._editing||UIUtils.isEditing()){return;}
...
contextMenu.defaultSection().appendItem(UIString.UIString('Reset Columns'),this._resetColumnWeights.bind(this));contextMenu.show();return;}
...
headerSubMenu.defaultSection().appendItem(UIString.UIString('Reset Columns'),this._resetColumnWeights.bind(this));const isContextMenuKey=(event.button===0);const gridNode=isContextMenuKey?this.selectedNode:this.dataGridNodeFromNode(target);if(isContextMenuKey&&this.selectedNode){const boundingRowRect=this.selectedNode.existingElement().getBoundingClientRect();if(boundingRowRect){const x=(boundingRowRect.right+boundingRowRect.left)/2;const y=(boundingRowRect.bottom+boundingRowRect.top)/2;contextMenu.setX(x);contextMenu.setY(y);}}
if(this._refreshCallback&&(!gridNode||gridNode!==this.creationNode)){contextMenu.defaultSection().appendItem(UIString.UIString('Refresh'),this._refreshCallback.bind(this));}
if(gridNode&&gridNode.selectable&&!gridNode.isEventWithinDisclosureTriangle(event)){if(this._editCallback){if(gridNode===this.creationNode){contextMenu.defaultSection().appendItem(UIString.UIString('Add new'),this._startEditing.bind(this,target));}else if(isContextMenuKey){const firstEditColumnIndex=this._nextEditableColumn(-1);if(firstEditColumnIndex>-1){const firstColumn=this.visibleColumnsArray[firstEditColumnIndex];if(firstColumn&&firstColumn.editable){contextMenu.defaultSection().appendItem(ls`Edit "${firstColumn.title}"`,this._startEditingColumnOfDataGridNode.bind(this,gridNode,firstEditColumnIndex));}}}else{const columnId=this.columnIdFromNode(target);if(columnId&&this._columns[columnId].editable){contextMenu.defaultSection().appendItem(UIString.UIString('Edit "%s"',this._columns[columnId].title),this._startEditing.bind(this,target));}}}
if(this._deleteCallback&&gridNode!==this.creationNode){contextMenu.defaultSection().appendItem(UIString.UIString('Delete'),this._deleteCallback.bind(this,gridNode));}
...
const CornerWidth=14;const Events={SelectedNode:Symbol('SelectedNode'),DeselectedNode:Symbol('DeselectedNode'),OpenedNode:Symbol('OpenedNode'),SortingChanged:Symbol('SortingChanged'),PaddingChanged:Symbol('PaddingChanged'),};const Order={Ascending:'sort-ascending',Descending:'sort-descending'};const Align={Center:'center',Right:'right'};const DataType={String:Symbol('String'),Boolean:Symbol('Boolean'),};const ColumnResizePadding=24;const CenterResizerOverBorderAdjustment=3;const ResizeMethod={Nearest:'nearest',First:'first',Last:'last'};class DataGridNode extends ObjectWrapper.ObjectWrapper{constructor(data,hasChildren){super();this._element=null;this._expanded=false;this._selected=false;this._dirty=false;this._inactive=false;this.key;this._depth;this._revealed;this._attached=false;this._savedPosition=null;this._shouldRefreshChildren=true;this._data=data||{};this._hasChildren=hasChildren||false;this.children=[];this.dataGrid=null;this.parent=null;this.previousSibling=null;this.nextSibling=null;this.disclosureToggleWidth=10;this.selectable=true;this._isRoot=false;this.nodeAccessibleText='';this.cellAccessibleTextMap=new Map();}
...
return cell;}
setCellAccessibleName(name,cell,columnId){this.cellAccessibleTextMap.set(columnId,name);for(let i=0;i<cell.children.length;i++){ARIAUtils.markAsHidden(cell.children[i]);}
ARIAUtils.setAccessibleName(cell,name);}
...
child._detach();child.resetNode();ArrayUtilities.removeElement(this.children,child,true);if(this.children.length<=0){this.setHasChildren(false);}}
...
class CreationDataGridNode extends DataGridNode{constructor(data,hasChildren){super(data,hasChildren);this.isCreationNode=true;}
makeNormal(){this.isCreationNode=false;}}
class DataGridWidget extends Widget.VBox{constructor(dataGrid){super();this._dataGrid=dataGrid;this.element.appendChild(dataGrid.element);this.setDefaultFocusedElement(dataGrid.element);}
...
this._visibleNodes=visibleNodes;this.dispatchEventToListeners(Events$1.ViewportCalculated);}
...
const Events$1={ViewportCalculated:Symbol('ViewportCalculated')};class ViewportDataGridNode extends DataGridNode{constructor(data,hasChildren){super(data,hasChildren);this._stale=false;this._flatNodes=null;this._isStriped=false;}
...
if(child.parent!==this){throw'removeChild: Node is not a child of this node.';}
ArrayUtilities.removeElement(this.children,child,true);child._unlink();if(!this.children.length){this.setHasChildren(false);}
...
var ViewportDataGrid$1=Object.freeze({__proto__:null,ViewportDataGrid:ViewportDataGrid,Events:Events$1,ViewportDataGridNode:ViewportDataGridNode});class SortableDataGrid extends ViewportDataGrid{constructor(dataGridParameters){super(dataGridParameters);this._sortingFunction=SortableDataGrid.TrivialComparator;this.setRootNode((new SortableDataGridNode()));}
...
class SortableDataGridNode extends ViewportDataGridNode{constructor(data,hasChildren){super(data,hasChildren);}
...
var SortableDataGrid$1=Object.freeze({__proto__:null,SortableDataGrid:SortableDataGrid,SortableDataGridNode:SortableDataGridNode});class ShowMoreDataGridNode extends DataGridNode{constructor(callback,startPosition,endPosition,chunkSize){super({summaryRow:true},false);this._callback=callback;this._startPosition=startPosition;this._endPosition=endPosition;this._chunkSize=chunkSize;this.showNext=createElement('button');this.showNext.setAttribute('type','button');this.showNext.addEventListener('click',this._showNextChunk.bind(this),false);this.showNext.textContent=UIString.UIString('Show %d before',this._chunkSize);this.showAll=createElement('button');this.showAll.setAttribute('type','button');this.showAll.addEventListener('click',this._showAll.bind(this),false);this.showLast=createElement('button');this.showLast.setAttribute('type','button');this.showLast.addEventListener('click',this._showLastChunk.bind(this),false);this.showLast.textContent=UIString.UIString('Show %d after',this._chunkSize);this._updateLabels();this.selectable=false;}
...
this.showAll.textContent=UIString.UIString('Show all %d',totalSize);}
...
dispose(){}}
var ShowMoreDataGridNode$1=Object.freeze({__proto__:null,ShowMoreDataGridNode:ShowMoreDataGridNode});export{DataGrid$1 as DataGrid,ShowMoreDataGridNode$1 as ShowMoreDataGridNode,SortableDataGrid$1 as SortableDataGrid,ViewportDataGrid$1 as ViewportDataGrid};