import{InspectorFrontendHost}from'../host/host.js';class ServiceManager{createRemoteService(serviceName){if(!this._remoteConnection){const url=Root.Runtime.queryParam('service-backend');if(!url){console.error('No endpoint address specified');return(Promise.resolve(null));}
this._remoteConnection=new Connection(new RemoteServicePort(url));}
return this._remoteConnection._createService(serviceName);}
createAppService(appName,serviceName){let url=appName+'.js';const remoteBase=Root.Runtime.queryParam('remoteBase');const debugFrontend=Root.Runtime.queryParam('debugFrontend');const isUnderTest=InspectorFrontendHost.isUnderTest();const queryParams=[];if(remoteBase){queryParams.push('remoteBase='+remoteBase);}
...
class Service{constructor(connection,serviceName,objectId){this._connection=connection;this._serviceName=serviceName;this._objectId=objectId;this._notificationHandlers=new Map();}
...
return false;});}}
var ServiceManager$1=Object.freeze({__proto__:null,ServiceManager:ServiceManager,Connection:Connection,Service:Service,RemoteServicePort:RemoteServicePort,WorkerServicePort:WorkerServicePort});const serviceManager=new ServiceManager();export{ServiceManager$1 as ServiceManager,serviceManager};